...
(defn ->post
  [{:keys [garden-path]
    :as   post}]
  (let [path                      (:blog.post/path post garden-path)
    ...
    (cond-> post
      true (assoc
             ...
             :blog.post/fname fname
             :blog.post/parent-fname (fs/file-name (fs/parent path))
             :blog.post/last-modified (fs/file-time->millis (fs/last-modified-time path))
             )

      (:blog.post/path post) (assoc :blog.post/relative-url
                                    (string/replace path config/docs-dir ""))
      (not (nil? date))      (assoc :blog.post/date-created date))
    ))
...
(defn sort-latest-created-first [posts]
  (->> posts
       (sort-by :blog.post/date-created dates/sort-latest-first)))

(defn sort-oldest-created-first [posts]
  (->> posts
       (sort-by :blog.post/date-created dates/sort-chrono)))
...
(defn posts-in-dir [directory]
  (->> (fs/glob (str directory) "*.md")
       (map (fn [path] (post/->post {:blog.post/path (str path)})))))
...
(defn org-root-files []
  (->>
    ...
    (map (fn [path] (post/->post {:garden-path path})))))
